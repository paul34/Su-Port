...
/**
 * 
 * This class has a bash script that calls Iptables to allow POP e-mail traffic on to the home computer
 ...
 * 
 */
public class allowPOP implements Runnable
{
	//Retrieve user data
	String ethNum= iptables.poptf.getText();
...
"/bin/sh",
"-c",
"iptables -A INPUT -i " + ethNum + " -p tcp --dport 110 -m state --state NEW,ESTABLISHED -j ACCEPT"
...
"/bin/sh",
"-c",
"iptables -A OUTPUT -o " + ethNum + " -p tcp --sport 110 -m state --state ESTABLISHED -j ACCEPT"
...
iptables.textArea.setText("Allow POP Rule Applied");
